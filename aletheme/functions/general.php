...
if ( function_exists('register_sidebar') ) {
    ...
    register_sidebar(array(
        'name' => 'Main Sidebar',
        'id' => 'main-sidebar',
        'description' => 'Appears as the right sidebar on Custom Template with Sidebar pages',
...
function ale_get_breadcrumbs() {
    ...
    $text['home']     = esc_html__('Home','gardener');
    $text['category'] = esc_html__('Archive','gardener').' "%s"';
    $text['search']   = esc_html__('Search results','gardener').' "%s"';
    $text['tag']      = esc_html__('Tag','gardener').' "%s"';
    $text['author']   = esc_html__('Author','gardener').' %s';
    $text['404']      = esc_html__('Error 404','gardener');
    ...
    if (is_home() || is_front_page()) {
        ...
        if(get_option( 'page_for_posts' )){
                echo '<div class="breadcrumbs"><a href="' . esc_url($home_link) . '">' . esc_attr($text['home']) . '</a>'.ale_wp_kses($delimiter).' '.__('Blog','gardener').'</div>';
        }
    }
    else {
        ...
        if ( get_query_var('paged') ) {
            ...
            echo esc_html__('Page','gardener') . ' ' . get_query_var('paged');
            if ( is_category() || is_day() || is_month() || is_year() || is_search() || is_tag() || is_author() ) echo ')';
        }
