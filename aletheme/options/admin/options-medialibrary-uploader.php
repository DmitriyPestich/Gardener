...
if ( ! function_exists( 'optionsframework_medialibrary_uploader' ) ) {
	...
	function optionsframework_medialibrary_uploader( $_id, $_value, $_mode = 'full', $_desc = '', $_postid = 0, $_name = '') {
		...
		if ($_name != '') {
			$name = $option_name.'['.esc_attr($id).']['.esc_attr($_name).']';
		}
		else {
			$name = $option_name.'['.$id.']';
		}
		...
		if(function_exists('optionsframework_mlu_init')){
			$output .= '<input id="upload_' . $id . '" class="upload_button button" type="button" value="' . esc_html__( 'Upload', 'gardener' ) . '" rel="' . $int . '" />' . "\n";
		} else {
		...
		if ( $value != '' ) { 
			...
			if ( $image ) {
				...
				$title = esc_html__( 'View File', 'gardener' );
...
if ( ! function_exists( 'optionsframework_mlu_modify_tabs' ) ) {
	...
	function optionsframework_mlu_modify_tabs ( $tabs ) {
		$tabs['gallery'] = str_replace( esc_html__( 'Gallery', 'gardener' ), esc_html__( 'Previously Uploaded', 'gardener' ), $tabs['gallery'] );
		return $tabs;
	}
}