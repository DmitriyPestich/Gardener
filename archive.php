    ...
    <section class="site_container">
        <div class="page_heading font_two">
            <h2 class="page_title wrapper">
                <?php
                if( is_day() ) :
                    $ale_title_for_heading = sprintf(esc_html__('Daily Archive: %s', 'gardener'), get_the_date('d M') );
                elseif(is_month()):
                    $ale_title_for_heading = sprintf(esc_html__('Mounthly Archive: %s', 'gardener'), get_the_date('M') );
                elseif(is_year()) :
                    $ale_title_for_heading = sprintf(esc_html__('Yearly Archive: %s', 'gardener'), get_the_date('Y') );
                else :
                    $ale_title_for_heading = esc_html__('Archive', 'gardener');
                endif;

                echo ale_wp_kses ($ale_title_for_heading);
                ?>
            </h2>
        </div>
        <div class="blog_cats">
